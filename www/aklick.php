...
if(count($_POST)>0) {
	...
	for($i = 0;$i<=$num;$i++) {
		if($user_ids[$id[$af[$i]]] == $_SESSION['user_id'] || $user->typ == 2) {
			$afick_am .= "<a href='$_SERVER[PHP_SELF]?edit=".$id[$af[$i]]."'>".trim($af[$i])."</a> ";	
		} else {
			$afick_am .= trim($af[$i])." ";	
		}
	}
	...
	if($user_ids[$id[$am[0]]] == $_SESSION['user_id'] || $user->typ == 2) {
		$afick_am .= "<a href='$_SERVER[PHP_SELF]?edit=".$id[$am[0]]."'>".$am[0]."</a>";
	} else {
		$afick_am .= $am[0];
	}
	...
	VALUES
	('$_SESSION[user_id]','$afick_am','$aklick',now())";
	$db->query($sql,__FILE__,__LINE__);
	header("Location: ".$_SERVER['PHP_SELF']."?".session_name()."=".session_id());
...
<table class='border'><tr><td align='left'>
<B>Afick:</B>
</td><td align='left'>
<input type='text' class='text' name='afick' size='80' tabindex='0'>
</td></tr><tr>
<td align='left'>
<a href='$_SERVER[PHP_SELF]?new=1'>Neues Wort</a>
</td>
<td align='right'>
<input type='submit' value='maaaaach' name='del' class='button'>
</td></tr>
</table>
</from><br />";
