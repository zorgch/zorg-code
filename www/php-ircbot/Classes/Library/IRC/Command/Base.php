<?php
    ...
    abstract class Base {
       ...
       protected function say($msg) {
			// Split into nickname and user/host name.
			$privNick = explode("!", $this->privSource);
			// We only want the nickname.
			$privNick = $privNick[0];

			// If the message was a private one then forward back to the messaging user rather than ourself!
			$toNick = ($this->source == $this->getNick) ? $privNick : $this->source;

            $this->connection->sendData(
                    'PRIVMSG '. $toNick .' :'. $msg
            );

            /*Old PRIVMSG-Code: https://github.com/ElvenSpellmaker/IRC-Bot/commit/d7912ce1ae3713c5966ff6503772ea4ed0a7e7d9
	        $this->connection->sendData(
