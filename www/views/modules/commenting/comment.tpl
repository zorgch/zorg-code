...
{/if}
{/nocache}
<table class="forum" itemid="{$smarty.const.SITE_URL}{comment_get_link board=$comment_data.board thread_id=$comment_data.id}" itemscope itemtype="http://schema.org/DiscussionForumPosting">
	<tr>
			...
			<table style="table-layout: fixed;background-color: #{comment_colorfade depth=0 color=$comment_color};">
				<tr class="tiny">
					<td class="forum comment meta left" style="width: 70%;">
						...
						&nbsp;by&nbsp;<span itemprop="author" itemscope itemtype="http://schema.org/Person">{$comment_data.user_id|userpage:0}</span>
