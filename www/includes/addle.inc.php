...
function getOpenAddleGames($userID) {
	...
	if(isset($userID)) {
		...
		$sql = "select id from addle where ( (player1 = $userID and nextturn = 1) or ( player2 = $userID and nextturn = 2) ) and finish = 0";
		$result = $db->query($sql);
...
function highscore_dwz ($anzahl) {
	...
	while ($d = mysql_fetch_array($e)) {
		...
		if ($d[tendenz] > 0) $pic = "/images/arr_up.gif";
		elseif ($d[tendenz] < 0) $pic = "/images/arr_down.gif";
...
function _update_dwz ($id) {
   ...
   $prev_score_2 = $prev_score_1 = BASE_POINTS;
   
   $e = $db->query("SELECT * FROM addle WHERE id=$id AND finish=1", __FILE__, __LINE__);
   $d = $db->fetch($e);
   ...
   $e = $db->query("SELECT * FROM addle_dwz WHERE user=$d[player1]", __FILE__, __LINE__);
   $d1 = $db->fetch($e);
   if ($d1) {
   	$dwz1 = $d1[score];
   	$prev_score_1 = $dwz1;
   }
   else $dwz1 = BASE_POINTS;
   $e = $db->query("SELECT score FROM addle_dwz WHERE user=$d[player2]", __FILE__, __LINE__);
   $d2 = $db->fetch($e);
   if ($d2) {
   	$dwz2 = $d2[score];
   	$prev_score_2 = $dwz2;
   }
   else $dwz2 = BASE_POINTS;
   
   $prob1 = 1 / (pow(10, (($dwz2 - $dwz1) / 400)) + 1) ;
   $prob2 = 1 / (pow(10, (($dwz1 - $dwz2) / 400)) + 1) ;
   
   $dif1 = round (MAX_POINTS_TRANSFERABLE * ($p1 - $prob1));
   $dif2 = round (MAX_POINTS_TRANSFERABLE * ($p2 - $prob2));
   
   $dwz1 += $dif1;
   $dwz2 += $dif2;
   
   if ($d1) $db->query("UPDATE addle_dwz SET score=$dwz1, prev_score=$prev_score_1 WHERE user=$d[player1]", __FILE__, __LINE__);
   else $db->query("INSERT INTO addle_dwz (user, score, prev_score) VALUES ($d[player1], $dwz1, $prev_score_1)", __FILE__, __LINE__);
   ...
   // rank update
   $e = $db->query("SELECT * FROM addle_dwz ORDER BY score DESC", __FILE__, __LINE__);
   $i = 1;
   $prev_score = 0;
   $rank = 0;
   while ($upd = $db->fetch($e)) {
   	...
   	$db->query("UPDATE addle_dwz SET rank=$rank $prev_rank WHERE user=$upd[user]", __FILE__, __LINE__);
...
$sql = "SELECT * FROM addle WHERE ((player1 = 59 OR player2 = 59) AND (player1 = 1 OR player2 = 1) AND finish = 0)";
...
while($rs = $db->fetch($result)) {
	...
	if($rs['player1'] == 59 && $nextturn == 1) {
		...
		$checker = 1;
		$my_score = "score1";
	} elseif($rs['player2'] == 59 && $nextturn == 2) {
		$mode = 2;
	...
	if ($checker == 1) {
		$sql = "UPDATE addle set data = '$new_data[game_data]', nextturn = $new_nextturn, nextrow = '$new_data[row]', 
		 	$my_score = $new_data[score], date = unix_timestamp(now())";
		if($new_data['row'] != 23) {
			$sql_add = "";
		} else {
			$sql_add = ", finish = 1 ";
