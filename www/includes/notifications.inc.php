...
class Notification
{
	...
	public function send($user_id, $notification_source, $content)
	{
		...
		if (DEVELOPMENT) error_log(sprintf('[DEBUG] <%s:%d> passed parameters: %d | %s | %s', __METHOD__, __LINE__, $user_id, $notification_source, $content));
	...
	private function get($user_id)
	{
		...
		if (!is_numeric($user_id) || $user_id <= 0 || is_array($user_id) || $user->id2user($content['from_user_id']) !== false) {
	...
	private function sendEmailNotification($from_user_id, $to_user_id, $titel, $text)
	{
		...
		if (!empty($to_user_id) && is_numeric($to_user_id))
		{
			...
			if (!empty($empfaengerMail))
			{
				...
				$subject = 	remove_html( t('email-notification-subject', 'messagesystem', [ $senderName, SITE_HOSTNAME ]), ENT_DISALLOWED, 'UTF-8' );
				...
				try {
					...
					if (mail("$empfaengerName <$empfaengerMail>", $subject, $body, $header))
					{
						return true;
					} else {
						error_log(sprintf('[WARN] <%s:%d> mail() ERROR to %s', __METHOD__, __LINE__, $empfaengerMail));
						return false;
					}
