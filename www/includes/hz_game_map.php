...
if (empty($gameid))
{
	...
	if ($d) {
		...
		$e = $db->query('SELECT * FROM hz_tracks t WHERE t.game='.$d['id'].' AND t.player="z" ORDER BY nr DESC LIMIT 0,1', __FILE__, __LINE__, 'SELECT FROM hz_tracks');
		$last_track = $db->fetch($e);

		$e = $db->query('SELECT p.*, s.x, s.y FROM hz_players p, hz_stations s WHERE p.game='.$d['id'].' AND s.id=p.station AND s.map='.$d['map'].' ORDER BY p.type ASC',
		...
		$e = $db->query('SELECT s.x, s.y, t.station FROM hz_tracks t, hz_stations s WHERE t.game='.$d['id'].' AND s.id=t.station AND s.map='.$d['map'].' AND t.player="z"',
			__FILE__, __LINE__, 'SELECT FROM hz_tracks');
		while ($t = $db->fetch($e)) {
			draw_z_seen($im, $t['x'], $t['y'], 0);
		}

		if ($d['state'] == "finished") {
			$e = $db->query('SELECT s.x, s.y FROM hz_players z, hz_stations s WHERE z.game='.$d['id'].' AND z.type="z" AND s.id=z.station AND s.map='.$d['map'],
				__FILE__, __LINE__, 'SELECT FROM hz_players');
			$f = $db->fetch($e);
