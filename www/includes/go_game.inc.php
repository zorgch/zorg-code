...
define("LASTSTONE", imagecreatefrompng(GOIMGPATH."go_last.png"));
	...
	function go_new_game ($opponent, $size, $handicap) {
		...
		if (!$user->id) user_error("You have to be logged in to start a new GO game.", E_USER_ERROR);
		    ...
		    else user_error("The selected opponent doesn't exist.", E_USER_ERROR);
		}
	        else user_error("The selected size is invalid.", E_USER_ERROR);
	}
    ...
    function go_luck($gameid)
    {
	    ...
	    if (!$user->id) user_error("You have to be logged in to wish luck.", E_USER_ERROR);

	    $game = readGame($gameid);
	    if (!$game)
	    {
			user_error("Invalid game_ID: '$gameid'");
			return;
	    }
    ...
    function go_thank($gameid)
    {
	    ...
	    if (!$user->id) user_error("You have to be logged in.", E_USER_ERROR);
	...
	function go_move($which, $gameid)
	{
	    ...
	    {
			user_error("Game is not in 'running' state!");
			return;
	    }
	    
	    if ($game['nextturn'] != $user->id)
	    {
			user_error("Du bisch noed dra, floip!");
			return;
	    }
	    ...
	    {
			user_error("Koordinaten ausserhalb des Spielfeldes: $which (x=$x, y=$y)");
			return;
	    }
	    ...
	    {
			user_error("Suizid ist nicht möglich. (Aber probiere chammers jo, gell)");
			return;
	    }
	    
	    if ($game['ko_sit'] == $which && count($hit_area) == 1)
	    {
			user_error("KO-Situation! Du kannst diesen Stein in dieser Runde nicht schlagen!");
			return;
	    }
	...
	function nextstone_map($gameid)
	{
		...
		if ($game['board'][$which] == 1 || $game['board'][$which] == 2){
		    ...
		    $msg = 'Gebietssteine: '.count($area).', Freiheiten: '.count($freedoms);
		    $islink = false;
		}
			...
			if ($ko_sit == $which && $countHit == 1){
			    $msg = 'KO Situation. Du kannst diesen Stein in dieser Runde nicht schlagen!';
			    $islink = false;
		        }
		        else {
			    // check if you can hit the opponent
         		    $msg = 'Jawoll, friss de Chaib!';
			    $islink = true;
			}
		    }
		    else if (check_suicide($which, $game)){
			// check for suicide
			$msg = 'Spinnsch enart, chasch di jo noed selber umloh!';
			$islink = false;
		    }
		    else{
			$msg = 'Tu in daaaaa rein!';
			$islink = true;
		    }
		}
		else continue;
	    ...
	    if ($game)
	    {
			...
			if($user->id != $game['nextturn'])
			{
				Messagesystem::sendMessage(
					...
					$game['nextturn'],
					'[Go] Du bist dran!',
					sprintf('<i>Ich habe meinen GO-Zug gemacht, du bist jetzt dran in <a href="%s/?tpl=699&game=%d">unserem GO Spiel</a></i>', SITE_URL, $game['id']),
					$game['nextturn']
				);
			}
	    }
	...
	function go_count($which, $gameid){
        ...
        {
			user_error("Game is not in 'counting' state!");
			return;
	    }
	    ...
	    {
			user_error("Feld $which (x=$x, y=$y) ist leer!");
            return;
	    }
        ...
        if ($oldone == 1) $newone = 3;
      		...
      		user_error("Invalid data state: $oldone");
      		return;
        }
