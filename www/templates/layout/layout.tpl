...
{include file='file:layout/head.tpl' scope=parent}
{if !$smarty.get.tpleditor}
	{include file=$tplroot.id scope=parent}
{else}
	{if $user->typ >= $smarty.const.USER_USER}
		{include file='file:layout/pages/tpleditor.tpl' scope=parent}
	{else}
		{assign_array var=error value="array('type'=>'warn', 'title'=>'Nice try', 'message'=>'Permissions denied, du Sack!', 'dismissable'=>'false')"} 
		{include file='file:layout/elements/block_error.tpl' scope=parent}
	{/if}
{/if}
{include file='file:layout/footer.tpl' scope=parent}