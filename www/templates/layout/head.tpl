...
<html>
	<head>
		...
		{if $tplroot.page_title == 'Home'}<!-- Redirect Mobile Devices, kudos to http://detectmobilebrowsers.com/ -->
		...
		if (isMobile)
			if (confirmPopup("[z]Chat anzeigen?")) window.location="mobilezorg-v2/";
		{/literal}</script>{/if}
	</head>
